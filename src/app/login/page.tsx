...
export default function LoginPage() {
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen px-4 overflow-hidden bg-slate-50">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-8 w-full max-w-4xl z-10 py-10">
        ...
        {/* Dev Credentials Helper (Only visible in Development mode) */}
        {process.env.NODE_ENV === "development" && (
          <div className="w-full max-w-sm bg-white border border-slate-200 shadow-xl rounded-3xl p-6 space-y-4 animate-in fade-in slide-in-from-right-4 duration-350 shrink-0">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              <span className="text-lg">🛠️</span>
              <div>
                <h4 className="text-sm font-bold text-slate-800 leading-none">Developer Helpers</h4>
                <span className="text-[10px] text-violet-600 font-bold uppercase tracking-wider">Acme & Global Seed Accounts</span>
              </div>
            </div>
            ...
            <div className="space-y-3.5">
              ...
              <div className="space-y-1.5">
                <span className="block text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Tenant: acme (UK VAT & Base GBP)</span>
                <div className="grid grid-cols-1 gap-2">
                  {[
                    { username: "admin", role: "CompanyAdmin" },
                    ...
                    <button
                      ...
                      className="p-2.5 bg-slate-50 border border-slate-200/80 rounded-xl hover:border-violet-300 hover:bg-violet-50/20 text-left transition-all group flex justify-between items-center cursor-pointer select-none"
                    >
                      <div>
                        <span className="block text-xs font-bold text-slate-700 leading-none group-hover:text-violet-750">{u.username}</span>
                        <span className="block text-[8px] text-slate-400 font-bold uppercase tracking-wider mt-1">{u.role}</span>
                      </div>
                      <span className="text-[10px] font-mono text-slate-400 font-semibold bg-white border border-slate-200 px-1.5 py-0.5 rounded group-hover:border-violet-200 group-hover:text-violet-600">password123</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1.5 pt-2 border-t border-slate-100">
                <span className="block text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Tenant: global (US Dollar)</span>
                <div className="grid grid-cols-1 gap-2">
                  {[
                    { username: "admin", role: "CompanyAdmin" },
                    { username: "purchasing", role: "Sales" }
                  ].map(u => (
                    <button
                      key={u.username}
                      type="button"
            ...
            <p className="text-[9px] text-slate-400 leading-normal pt-1.5 border-t border-slate-100">
              💡 Clicking a credential card will automatically populate the login fields for quick testing.
            </p>
          </div>
        )}
      </div>
