...
import { Puzzle, ShieldAlert, BadgeCheck, Loader2, ShieldCheck, Cloud, Settings, X, Landmark } from "lucide-react";
...
export default function PluginsSettingsPage() {
  ...
  const [isConfiguringActive, setIsConfiguringActive] = useState(false);
  const [connString, setConnString] = useState("");
  const [containerName, setContainerName] = useState("");
  const [cdnEndpoint, setCdnEndpoint] = useState("");
  const [validationError, setValidationError] = useState<string | null>(null);

  // BNK configuration modal states
  const [showBankConfigModal, setShowBankConfigModal] = useState(false);
  const [isConfiguringBankActive, setIsConfiguringBankActive] = useState(false);
  ...
  const handleToggle = async (code: string, currentIsSubscribed: boolean) => {
    ...
    if (code === "BNK" && !currentIsSubscribed) {
      setIsConfiguringBankActive(false);
      setBankValidationError(null);
      setSelectedBank("Barclays");
      setBankClientId("");
      setBankSecretKey("");
  ...
  const handleOpenModifyConfig = (plugin: any) => {
    if (plugin.code === "CDN") {
      ...
      setShowConfigModal(true);
    } else if (plugin.code === "BNK") {
      setIsConfiguringBankActive(true);
      setBankValidationError(null);
      try {
        if (plugin.configurationSettingsJson) {
          const settings = JSON.parse(plugin.configurationSettingsJson);
          setSelectedBank(settings.bank || "Barclays");
          setBankClientId(settings.clientId || "");
          setBankSecretKey(settings.clientSecret || "");
        } else {
          setSelectedBank("Barclays");
          setBankClientId("");
          setBankSecretKey("");
  ...
  const handleSaveBankConfig = async (e: React.FormEvent) => {
    ...
    if (!selectedBank.trim() || !bankClientId.trim() || !bankSecretKey.trim()) {
      setBankValidationError("All bank connection credentials are required.");
      return;
    }

    const config = {
      bank: selectedBank.trim(),
      clientId: bankClientId.trim(),
      clientSecret: bankSecretKey.trim()
    };
    const configJson = JSON.stringify(config);

    setUpdatingCode("BNK");
    try {
      if (isConfiguringBankActive) {
        const success = await updatePluginConfig("BNK", configJson);
        if (success) {
          alert("Open Banking feed configuration successfully updated!");
          setShowBankConfigModal(false);
        } else {
          setBankValidationError("Failed to update bank connection settings.");
        }
      } else {
        const isSubscribed = await togglePlugin("BNK", configJson);
        if (isSubscribed) {
          setShowBankConfigModal(false);
        } else {
          setBankValidationError("Failed to activate banking feed module.");
        }
      }
    } catch (err: any) {
      setBankValidationError(err.response?.data?.message || "An unexpected error occurred.");
    } finally {
      setUpdatingCode(null);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-660 text-sm mt-1">Manage paid plugin extensions. Enabling these modules unlocks database columns and custom API routing.</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="mt-8 space-y-2">
                {plugin.isSubscribed && (plugin.code === "CDN" || plugin.code === "BNK") && (
                  <button
                  ...
                  >
                    <Settings className="w-3.5 h-3.5" /> Modify {plugin.code === "CDN" ? "Storage" : "Bank"} Settings
                  </button>
                )}
                {plugin.isSubscribed ? (
                  <button
                  ...
                  <button
                    ...
                    disabled={isUpdating || !isAdmin}
                    className="w-full py-2.5 px-4 rounded-xl text-xs font-semibold bg-violet-600 hover:bg-violet-550 text-white shadow-md shadow-violet-500/10 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer active:scale-95"
                  >
                    {isUpdating ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                      plugin.code === "CDN" || plugin.code === "BNK" ? "Configure & Activate" : "Subscribe & Activate"
                    )}
                  </button>
                )}
              </div>
            ...
            <form onSubmit={handleSaveConfig} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full mt-1 text-xs px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:bg-white focus:border-violet-500 text-slate-880"
                  />
                </div>
                <div>
              ...
              <div className="flex gap-3 justify-end pt-3 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => setShowConfigModal(false)}
                  className="px-4 py-2 border border-slate-200 text-slate-550 hover:text-slate-750 text-xs font-semibold rounded-lg cursor-pointer"
                >
                  Cancel
                </button>
                <button
      ...
      {/* BNK UK Bank Connections Configuration Modal */}
      {showBankConfigModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4">
          <div className="w-full max-w-md p-6 bg-white border border-slate-200 rounded-2xl shadow-2xl space-y-5 animate-in zoom-in-95 duration-150">
            <div className="flex justify-between items-center border-b border-slate-200 pb-3">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                {isConfiguringBankActive ? "Modify Bank feed Credentials" : "Setup UK Bank credentials"}
              </h3>
              <button 
            ...
            {bankValidationError && (
              <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-100 text-rose-650 text-xs leading-relaxed">
                {bankValidationError}
              </div>
            )}
            ...
            <form onSubmit={handleSaveBankConfig} className="space-y-4">
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-500 block">Select UK Financial Institution</label>
                <select
                  value={selectedBank}
                  onChange={(e) => setSelectedBank(e.target.value)}
                  className="w-full mt-1.5 text-xs px-3 py-2 bg-slate-50 border border-slate-205 rounded-lg focus:outline-none focus:bg-white focus:border-violet-500 text-slate-800 font-semibold cursor-pointer"
                >
                  <optgroup label="CMA9 (Mandated Banks)">
                    <option value="Barclays">Barclays</option>
                    <option value="HSBC Group">HSBC Group (includes HSBC Personal, Business, Kinetic)</option>
                    <option value="Lloyds Banking Group">Lloyds Banking Group (includes Lloyds Bank, Halifax, Bank of Scotland)</option>
                    <option value="NatWest Group">NatWest Group (includes NatWest, Royal Bank of Scotland, Coutts)</option>
                    ...
                    <option value="Allied Irish Bank">Allied Irish Bank (AIB Group UK / AIB NI)</option>
                    <option value="Bank of Ireland (UK)">Bank of Ireland (UK)</option>
                  </optgroup>
                  <optgroup label="Digital & Challenger Banks">
                    <option value="Monzo">Monzo</option>
                    <option value="Starling Bank">Starling Bank</option>
                    <option value="Revolut">Revolut</option>
                    <option value="First Direct">First Direct</option>
                    <option value="Cater Allen">Cater Allen (Santander subsidiary)</option>
                    <option value="Virgin Money">Virgin Money</option>
                    <option value="TSB">TSB</option>
                    <option value="Zempler Bank">Zempler Bank (formerly Arro / Cashplus)</option>
                  </optgroup>
                  <optgroup label="Building Societies & Specialist Banks">
                    <option value="Clydesdale Bank">Clydesdale Bank</option>
                    <option value="Yorkshire Bank">Yorkshire Bank</option>
                    <option value="Cater Allen Private Bank">Cater Allen Private Bank</option>
                    <option value="Mettle">Mettle (NatWest business app)</option>
                  </optgroup>
                </select>
              </div>

              <div>
                <label className="text-[10px] font-bold uppercase text-slate-500 block">Open Banking API Client ID</label>
                <input
                  type="text"
                  required
                  value={bankClientId}
                  onChange={(e) => setBankClientId(e.target.value)}
                  placeholder="e.g. client_id_barclays_99218"
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-500 block">API Private Access Secret / Private Key</label>
                <input
                  type="password"
                  required
                  value={bankSecretKey}
              ...
              <div className="flex gap-3 justify-end pt-3 border-t border-slate-200">
                ...
                >
                  {updatingCode === "BNK" ? (
                    ...
                    isConfiguringBankActive ? "Save Credentials" : "Authorize & Activate"
                  )}
                </button>
              </div>
            </form>
