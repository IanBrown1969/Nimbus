      ...
      <div className="flex items-center justify-between">
        ...
        {selectedAccId && (
          <button
            ...
            disabled={syncing}
            className="py-2.5 px-4 rounded-xl font-semibold bg-violet-600 hover:bg-violet-500 text-white shadow-md flex items-center gap-2 text-xs transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
          >
        ...
        {accounts.map(acc => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <Landmark className={`w-4.5 h-4.5 ${selectedAccId === acc.id ? "text-violet-650" : "text-slate-400"}`} />
            </div>
            <div className="mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-4">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {feed.length === 0 ? (
                <div className="text-slate-500 text-xs italic text-center py-8 border border-dashed border-slate-200 rounded-xl">
                          ...
                          {ledgerLines
                            ...
                            .map(match => (
                              <button
                                ...
                                className="py-1 px-2.5 rounded bg-cyan-600 hover:bg-cyan-500 text-white text-[10px] font-semibold transition-all shadow-sm cursor-pointer"
                              >
                                Match GL Code
                              </button>
                            ))}
          ...
          <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-4">
            ...
            <p className="text-xs text-slate-555">Unmatched journal entries in company ledger accounts (Debtors, Creditors, Expenses):</p>
            
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {ledgerLines
                .filter(ll => !feed.some(f => f.isReconciled && f.reconciledLedgerLineId === ll.id))
                .map(line => (
                  <div key={line.id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs flex justify-between items-center font-mono">
                    <div>
                      <strong className="text-slate-850 block font-sans truncate max-w-[200px]">{line.entryDescription}</strong>
                      <span className="text-[10px] text-slate-555 block mt-0.5">Account Code: {line.AccountCode}</span>
                    </div>
                    <div className="text-right">
                      <span className={`block font-bold ${line.debit > 0 ? "text-emerald-700" : "text-violet-750"}`}>
